...
import json
import random
import blockchain.ProofOfWork.nodelib as nodelib
import blockchain.ProofOfWork.viewlib as viewlib
...
def broadcast_transaction(transaction):
    for node in nodes:
        ...
        print(f"Broadcasted transaction to {node.address}")
    return True

def broadcast_block(block):
    recipients = []
    for i, node in enumerate(nodes):
        if node.rev_block(block):
            recipients.append(i)
    return recipients
...
def create_transactions():
    ...
    for i in range(nodeCount):
        if random.randint(0, 1) > 0:
            ...
            print(f"Transaction from {i} to {sel} with amount {amount}")
...
def mining_process(difficulty):
    for repeat in range(limit):
        ...
        #         print(f"Transaction from {tx['from']} to {tx['to']} with amount {tx['amount']}")
